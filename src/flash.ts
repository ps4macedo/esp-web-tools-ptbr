...
export const flash = async (
  ...
  try {
    ...
    fireStateEvent({
      ...
      details: { error: FlashErro.FAILED_INITIALIZING, details: err },
    });
  ...
  if (!build) {
    fireStateEvent({
      ...
      details: { error: FlashErro.NOT_SUPPORTED, details: chipFamily },
    });
    await hardReset(transport);
    await transport.disconnect();
    return;
  ...
  for (let part = 0; part < filePromises.length; part++) {
    try {
      ...
      fireStateEvent({
        ...
        message: err.message,
        details: {
          error: FlashErro.FAILED_FIRMWARE_DOWNLOAD,
          details: err.message,
        },
      });
      await hardReset(transport);
  ...
  fireStateEvent({
    state: FlashStateType.PREPARING,
    message: "Instalaration prepared",
    details: { done: true },
  });
  ...
  try {
    await esploader.writeFlash({
      ...
      reportProgress: (fileIndex: number, written: number, total: number) => {
        ...
        fireStateEvent({
          state: FlashStateType.WRITING,
          message: `Gravando progress: ${newPct}%`,
          details: {
            bytesTotal: totalSize,
    ...
    fireStateEvent({
      state: FlashStateType.ERROR,
      message: err.message,
      details: { error: FlashErro.WRITE_FAILED, details: err },
    });
    await hardReset(transport);
    await transport.disconnect();
    return;
