...
export class EwtInstalarDialog extends LitElement {
  ...
  protected render() {
    ...
    if (
      ...
      if (this._error) {
        [heading, content] = this._renderErro(this._error);
      } else {
        content = this._renderProgress("Conectaring");
      }
    } else if (this._state === "INSTALL") {
      [heading, content, allowClosing] = this._renderInstalar();
      ...
      [heading, content, allowClosing] = this._client
        ...
        : this._renderDashboardNãoImprov();
  ...
  _renderErro(label: string): [string, TemplateResult] {
    const heading = "Erro";
    const content = html`
      <ewt-page-message
        slot="content"
  ...
  _renderDashboard(): [string, TemplateResult, boolean] {
    ...
    content = html`
      <div slot="content">
        <ew-list>
          <ew-list-item>
            <div slot="headline">Conectared to ${this._info!.name}</div>
          ...
          ${this._isSameVersion
            ? html`
                <ew-list-item
                  type="button"
                  class="danger"
                  @click=${() => this._startInstalar(true)}
                >
                  ${listItemEraseUserData}
  ...
  _renderDashboardNãoImprov(): [string, TemplateResult, boolean] {
    const heading = this._manifest.name;
    let content: TemplateResult;
    let allowClosing = true;
    ...
    if (
      ...
      content = html`
        <div slot="content">
          <ewt-page-message
            .icon=${OK_ICON}
            label="Dispositivo conectado to the network!"
          ></ewt-page-message>
          ${showSetupLinks
            ? html`
                <ew-list>
      ...
      switch (this._client!.error) {
        case ImprovSerialErroState.UNABLE_TO_CONNECT:
          error = "Unable to connect";
          break;

        case ImprovSerialErroState.TIMEOUT:
          error = "Timeout";
          break;

        case ImprovSerialErroState.NO_ERROR:
        // Happens when list SSIDs not supported.
        case ImprovSerialErroState.UNKNOWN_RPC_COMMAND:
          break;
      ...
      content = html`
        ...
        <div slot="content">
          <div>Conectar your device to the network to start using it.</div>
      ...
      <div slot="actions">
        ...
        <ew-text-button
          @click=${() => {
            ...
            this._startInstalar(checkbox.checked);
          }}
        >
          Next
        </ew-text-button>
  ...
  _renderInstalar(): [string | undefined, TemplateResult, boolean] {
    let heading: string | undefined;
    let content: TemplateResult;
    ...
    if (!this._installConfirmed && this._isSameVersion) {
      ...
      content = html`
        ...
        <div slot="actions">
          <ew-text-button class="danger" @click=${this._confirmInstalar}>
            Erase User Data
          </ew-text-button>
        </div>
      `;
    } else if (!this._installConfirmed) {
      heading = "Confirm Instalaration";
      ...
      content = html`
        ...
        <div slot="actions">
          ...
          <ew-text-button @click=${this._confirmInstalar}>
            Instalar
          </ew-text-button>
        </div>
      `;
      ...
      if (this._installState.state === FlashStateType.FINISHED) {
        ...
        undeterminateLabel = "Instalaring";
      } else {
  ...
  private async _initialize(justInstalared = false) {
    if (this.port.readable === null || this.port.writable === null) {
      this._state = "ERROR";
      this._error =
        "Serial port is not readable/writable. Fechar any other application using it and try again.";
      return;
    }
    ...
    try {
      ...
      this._client = client;
      client.addEventListener("disconnect", this._handleDesconectar);
    } catch (err: any) {
      // Clear old value
      this._info = undefined;
      if (err instanceof PortNãotReady) {
        this._state = "ERROR";
        this._error =
          "Serial port is not ready. Fechar any other application using it and try again.";
      } else {
  ...
  private _startInstalar(erase: boolean) {
    this._state = "INSTALL";
    this._installErase = erase;
    this._installConfirmed = false;
  }

  private async _confirmInstalar() {
    ...
    // Fechar port. ESPLoader likes opening it.
  ...
  private _handleDesconectar = () => {
    this._state = "ERROR";
    this._error = "Desconectared";
  };
  ...
  private async _handleFechar() {
    if (this._client) {
      await this._closeClientWithoutEvents(this._client);
    }
    fireEvent(this, "closed" as any);
    document.body.style.overflow = this._bodyOverflow!;
    this.parentNãode!.removeChild(this);
  }
  ...
  private async _closeClientWithoutEvents(client: ImprovSerial) {
    client.removeEventListener("disconnect", this._handleDesconectar);
    await client.close();
  }
...
customElements.define("ewt-install-dialog", EwtInstalarDialog);

declare global {
  interface HTMLElementTagNameMap {
    "ewt-install-dialog": EwtInstalarDialog;
  }
}
