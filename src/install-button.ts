...
import type { EwtInstalarDialog } from "./install-dialog";
import { connect } from "./connect";

export class InstalarButton extends HTMLElement {
  ...
  public overrides: EwtInstalarDialog["overrides"];
  ...
  public connectedCallback() {
    ...
    if (!InstalarButton.isSupported || !InstalarButton.isAllowed) {
      this.toggleAttribute("install-unsupported", true);
      this.renderRoot.innerHTML = !InstalarButton.isAllowed
    ...
    if (
      ...
      sheet.replaceSync(InstalarButton.style);
      this.renderRoot.adoptedStyleSheets = [sheet];
    } else {
      const styleSheet = document.createElement("style");
      styleSheet.innerText = InstalarButton.style;
...
customElements.define("esp-web-install-button", InstalarButton);
